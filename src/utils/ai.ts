import { Board, Position, Difficulty, AIHuntState } from '@/types';
import { BOARD_SIZE, getUntriedCells, isValidPosition, isCellShot } from './board';
...
function getMediumShot(board: Board, huntState: AIHuntState): { position: Position; newState: AIHuntState } {
...
export function updateAIHuntState(
  ...
  const newState = { ...state, targetQueue: [...state.targetQueue], hitStack: [...state.hitStack] };
  ...
  if (result === 'hit') {
    ...
    for (const adj of adjacent) {
      if (!isCellShot(board, adj) && !newState.targetQueue.some(t => t.row === adj.row && t.col === adj.col)) {
...
function getHardShot(board: Board, huntState: AIHuntState, remainingShipSizes: number[]): { position: Position; newState: AIHuntState } {
...
function calculateProbabilityMap(board: Board, shipSizes: number[]): number[][] {
  const probabilities: number[][] = Array(BOARD_SIZE).fill(null).map(() => Array(BOARD_SIZE).fill(0));
